...
Partition_nav::Partition_nav(ros::NodeHandle nh) : m_initialized(false)
{
  ROS_INFO("Partition_nav constructed.");

  m_nh = nh;
  m_pub = m_nh.advertise<visualization_msgs::MarkerArray>("cell_partition", 1);
}

void Partition_nav::initialize(double x0, double y0, double x1, double y1,
                               double rc, double scanRange)
{
  ...
  ROS_INFO("Partition_nav initialized.");
...
void Partition_nav::drawPartition()
{
...
void Partition_nav::getNeighbors(int l, int k, double dist,
...
void Partition_nav::update(const nav_msgs::OccupancyGrid& map, double x,
                           double y)
{
...
Partition_nav::CellStatus
Partition_nav::calculateStatus(const nav_msgs::OccupancyGrid& map, int l, int k)
{
...
void Partition_nav::gridToWorld(int l, int k, double& xc, double& yc)
{
  double xLocal;
  double yLocal;
...
void Partition_nav::worldToGrid(double xc, double yc, int& l, int& k)
{
...
void Partition_nav::gridToLocal(int l, int k, double& xc, double& yc)
{
...
void Partition_nav::localToGrid(double xc, double yc, int& l, int& k)
{
...
bool Partition_nav::hasCompleteCoverage()
{
