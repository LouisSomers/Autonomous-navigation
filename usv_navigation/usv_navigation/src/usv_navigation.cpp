...
USVNav::USVNav() : m_mapInitialized(false)
{
  ROS_INFO("usv_navigation_node started.");
  ...
  // Set up subscribers
  ros::Subscriber mapSub = nh.subscribe("inflated_map", 1000, &USVNav::onMapReceived, this);
  ros::Subscriber track_pSub = nh.subscribe("datmo/box_kf",10,&USVNav::trackingCenter_p, this);

//  ros::Subscriber trackSub = nh.subscribe("datmo/box_kf",100,&USVNav::trackingCenter, this);
...
void USVNav::onMapReceived(const nav_msgs::OccupancyGrid& grid)
{
  if (!m_mapInitialized)
    m_mapInitialized = true;
  m_partition.update(grid, m_pose.x, m_pose.y);
}
//void USVNav::trackingCenter(const datmo::Track& track) { track_center = track; }



void USVNav::mainLoop(ros::NodeHandle& nh)
{
...
bool USVNav::updateRobotPose(const tf2_ros::Buffer& tfBuffer)
{
...
void USVNav::trackingCenter_p(const datmo::TrackArray& track) { track_center = track; }

void USVNav::getTrackedTarget_p(double x, double y,float& track_x, float& track_y,std::vector<double>& trackedX,std::vector<double>& trackedY,std::vector<double>& vel_trackedX,std::vector<double>& vel_trackedY, std::vector<int>& vect_id, std::vector<double>& predX,std::vector<double>& predY){
...
void USVNav::block_cells(double x, double y){
...
void USVNav::drawReversePrediction(){
  double x_deblock, y_deblock;
...
void USVNav::findNextTargetWaypoint(double& xTarget, double& yTarget, double& xNext, double& yNext, double& yawNext, double& DistanceToTarget)
{
  ...
  Next_cellx = m_pose.x+5*cos(angle_of_line);
  Next_celly = m_pose.y+5*sin(angle_of_line);

  // a point further away (2 cells)
  Next_cellx2 = m_pose.x+8*cos(angle_of_line);
  Next_celly2 = m_pose.y+8*sin(angle_of_line);
  ...
  // There is an obstruction on the way to the point, so need to go around it
      ...
      do {
        ...
        Next_cellx_blocked_right = m_pose.x+4*(cos(angle_of_line +ii)); // was 4 30/04
        Next_celly_blocked_right = m_pose.y+4*(sin(angle_of_line +ii)); // was 8 30/04
       
        Next_cellx2_blocked_right = m_pose.x+8*(cos(angle_of_line +ii));
        Next_celly2_blocked_right= m_pose.y+8*(sin(angle_of_line +ii));
      ...
      do {
        ...
        Next_cellx_blocked_left = m_pose.x+4*(cos(angle_of_line +iii));
        Next_celly_blocked_left = m_pose.y+4*(sin(angle_of_line +iii));
       
        Next_cellx2_blocked_left = m_pose.x+8*(cos(angle_of_line + iii));
        Next_celly2_blocked_left= m_pose.y+8*(sin(angle_of_line +iii));
...
//void USVNav::driftthroughwind()
// based on the wind it is possible that the course to steer must be adapted
//void USVNav::setNextWaypoint(double& xTarget, double& yTarget double& XNextwaypoint, double& YnextWaypoint)

double USVNav::calculateI(PartitionBinn::CellStatus status, bool covered,
                                double x, double y)
{
...
double USVNav::calculateWeightSum(int l, int k)
{
...
double USVNav::calculateWeight(int l0, int k0, int l1, int k1)
{
  double x0, y0;
...
void USVNav::getNeighbors(int l, int k,
                                std::vector<PartitionBinn::Point>& neighbors)
{
...
void USVNav::getNeighbors2(int l, int k,
                                 std::vector<PartitionBinn::Point>& neighbors)
{
  neighbors.push_back({l + 1, k});
  neighbors.push_back({l - 1, k});
...
void USVNav::findNextCell(int& lNext, int& kNext, double& yawNext)
{
  // Get cell position HERE THE CELLS ARE FOUND FOR THE NEXT WAYPOINT
  int l, k;
  m_partition.worldToGrid(m_pose.x, m_pose.y, l, k);
...
double USVNav::scoreFunction(double neuralActivity, double yaw,
                                   double targetYaw)
{
  if (yaw < 0)
    yaw += 2 * M_PI;
...
void USVNav::evolveNeuralNetwork(double deltaTime)
{
...
void USVNav::publishGoal(double x, double y, double yaw)
{
  // Publish goal
